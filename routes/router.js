...
router.post("/signup", function (req, res) {
    ...
    connection.query(sql,[id],
        ...
        if (err) {
            ...
            let sql = `insert into user_list (post_id, password, name, division, linkcode) values (?, ?, ?, ?, 0)`
      ...
      function (err, result) {
        if (err) {
          ...
          connection.query(
            ...
            [car_id, car_div, car_type, car_birth, car_day, car_result, car_id],
...
router.post("/m_add_car", function (req, res) {
  ...
  const car_birth = req.body.car_birth;
  const car_day = req.body.car_day;
  const car_result = req.body.var_result;
  ...
  if (!req.session.loggedIn) {
    ...
    connection.query(
      `INSERT INTO car_list (car_id, car_div, car_type, car_birth, car_day, car_result) values (?, ?, ?, ?, ?, ?)`,
